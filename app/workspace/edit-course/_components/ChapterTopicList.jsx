...
function ChapterTopicList({course}) {
  ...
  return (
    <div>
        <h2 className='text-3xl font-bold mt-10'>Chapters & Topics</h2>
        <div className='flex flex-col items-center justify-center mt-10'>
            {courseLayout?.chapters.map((chapter,index)=>(
                <div key={index} className='flex flex-col items-center'>
                    <div className='p-4 border shadow rounded-xl bg-gray-400 text-white'>
                        ...
                        <span>Number Of Chapters:{chapter?.topics?.length}</span>
                        </h2>
                    </div>
                    <div>
